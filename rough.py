...
def analytic_to_bq_checking(ana_bucket_name, dataset, folder_path, project_id, records, bq_table_name):
    client = storage.Client(project_id)
    fs = GcsIO(client)
    ana_bucket = client.bucket(ana_bucket_name)
    blob_list_ana = client.list_blobs(ana_bucket, prefix=folder_path)
        ...
        try:
            ...
            if any(x in blob_ana.name for x in [dataset]):
                ...
                if dataset not in ["CAP_NVD_CAR", "CAP_NVD_LCV", "PRE_EMBARGO_J", "PRE_EMBARGO_LR"]:
                    ...
                    logger.info(f"File lines for {filename} is {ana_count}")

                    # Extract month column from the file name and format it for the query
                    month_col = datetime.strptime(blob_ana.name.split('/')[-2], '%Y%m%d').strftime('%Y-%m-%d')
                    ...
                    for row in rows:
                        bq_count = row.count
                        logger.info(f"BQ table count for {month_col} is {bq_count}")

                        if bq_count == ana_count:
                            bq_status = "Match"
                            reason = (f"{bq_status} : Bcoz ana_records({ana_count}) == BQ_count({bq_count}) = mismatch({bq_failed_count})")
                            logger.info(f"Data count matches for {filename} in analytical zone.")
                        else:
                            bq_status = "Not Match"
                            bq_failed_count = abs(ana_count - bq_count)
                            reason = (f"{bq_status} : Bcoz ana_records({ana_count}) != BQ_count({bq_count}) = mismatch({bq_failed_count})")
                            logger.info(f"Data count does not match for {filename} in analytical zone.")
...
def process_files(_, buckets_info, folder_path, dataset, project_id, bq_dataset_id):
    ...
    for zone, bucket_name in buckets_info.items():
        ...
        for file in files:
            ...
            if not bq_table_name:
                logger.warning(f"No matching table found for file: {filename}")
    ...
    if bq_table_name:
        logger.info(f"Calling analytic_to_bq_checking for table: {bq_table_name}")
        records = analytic_to_bq_checking(buckets_info["ANAL"], dataset, folder_path, project_id, records, bq_table_name)
    else:
        logger.warning("bq_table_name is empty or None, skipping analytic_to_bq_checking")
