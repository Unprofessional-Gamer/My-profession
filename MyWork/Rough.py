...
def activate_data_cleaning(bucket_name, base_path, file_name):
    ...
    with beam.Pipeline(options=options) as p:
        file_read = (p
                     | "Reading the file data" >> beam.io.ReadFromText(f"gs://{bucket_name}/{base_path}/RECEIVED/{file_name}")
    ...
    with beam.Pipeline(options=options) as p:
        ...
        for blob in blob_list:
            if blob.name.endswith('.csv'):
                ...
                check_status = True  # Assuming all checks pass for demonstration purposes
