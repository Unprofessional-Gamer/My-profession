...
def zip_and_transfer_csv_files(storage_client, raw_zone_bucket, raw_zone_folder_path, consumer_bucket_name, consumer_folder_path, archive_folder_path):
        ...
        for blob in blob_list:
            if blob.name.endswith('.csv'):
                ...
                if naming_convention is not None:
                    ...
                    archive_zip_name = os.path.join(archive_folder_path, naming_convention).replace("\\", "/")
                    ...
                    with zipfile.ZipFile(zip_buffer[archive_zip_name], 'a', zipfile.ZIP_DEFLATED) as zipf:
                        zipf.writestr(os.path.basename(blob.name), csv_data)
...
def copy_and_transfer_csv(raw_zone_bucket, sfg_base_path, consumer_bucket, consumer_folder_path, archive_folder_path, gfv_files):
    ...
    for blob in blob_list:
        ...
        if any(file_name.startswith(prefix) for prefix in gfv_files):
            ...
            consumer_bucket.blob(f"{archive_folder_path}/{file_name}").upload_from_string(blob.download_as_bytes(), 'text/csv')
            print(f"Moved {file_name} to {consumer_folder_path}/{file_name} and {archive_folder_path}/{file_name}")
        else:
            print(f"Ignored file: {file_name}")

def run_pipeline(project_id, raw_zone_bucket_name, sfg_base_path, consumer_folder_path, archive_folder_path, consumer_bucket_name):
...
if __name__ == "__main__":
    ...
    # Define the archive folder path with the current date in the format mm-yyyy
    current_date = datetime.now().strftime('%m-%Y')
    archive_folder_path = f"thParty/GFV/Monthly/{current_date}/archive"
    ...
    copy_and_transfer_csv(raw_zone_bucket, sfg_base_path, consumer_bucket, consumer_folder_path, archive_folder_path, gfv_files)
    print("**********CSV file copying completed**********")

    print("**********Files zipping started**********")
    zip_and_transfer_csv_files(storage_client, raw_zone_bucket, sfg_base_path, consumer_bucket_name, consumer_folder_path, archive_folder_path)
    print("**********Files zipping completed**********")

    run_pipeline(project_id, raw_zone_bucket_name, sfg_base_path, consumer_folder_path, archive_folder_path, consumer_bucket_name)
