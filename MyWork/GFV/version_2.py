...
def copy_and_transfer_csv(raw_zone_bucket, raw_zone_csv_path, consumer_bucket, consumer_folder_path):
    ...
    for blob in blob_list:
        file_name = blob.name.split("/")[-1]
        if file_name.startswith(("CPRRVN", "CPRRVU")):
            blob.copy_to_bucket(consumer_bucket, new_name=f"{consumer_folder_path}/{file_name}")
            print(f"Copied {file_name} directly to {consumer_folder_path}/{file_name}")
...
if __name__ == "__main__":
    ...
    print("**********GFV Files processing**********")
    copy_and_transfer_csv(raw_zone_bucket, raw_zone_folder_path, consumer_bucket, consumer_folder_path)
    print("**********GFV Files processing completed**********")
